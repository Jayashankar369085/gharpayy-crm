...
export function OwnerProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      // End-of-day auto-lock: after three warnings, unverified rooms become unsellable.
      if (phase === 'locked') {
        setRoomStatuses((prev) => {
          ...
          const next = prev.map((r) => {
            if (!r.verifiedToday && !r.lockedUnsellable) {
              changed = true;
              glueBus.publish({ type: 'owner.room.locked', roomId: r.roomId, propertyId: r.propertyId, ownerId: r.ownerId, reason: 'unverified_after_3_warnings' });
