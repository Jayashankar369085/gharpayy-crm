...
function CommandBridge() {
  ...
  function doImpersonate() {
    ...
    log({ actorId: "admin", actorName: "Admin", entityType: "session", entityId: impersonateId, action: "admin.impersonate", summary: `Impersonating ${t?.name ?? impersonateId}` });
  ...
  function togglePause() {
    ...
    log({ actorId: "admin", actorName: "Admin", entityType: "system", entityId: "sequences", action: next ? "admin.kill.on" : "admin.kill.off", summary: next ? "Paused all sequences" : "Resumed sequences" });
    toast[next ? "warning" : "success"](next ? "All sequences paused org-wide" : "Sequences resumed");
  }

  function sendBroadcast() {
    if (!broadcast.trim()) return;
    log({ actorId: "admin", actorName: "Admin", entityType: "broadcast", entityId: String(Date.now()), action: "admin.broadcast", summary: `Broadcast → ${tcms.length} TCMs: ${broadcast.slice(0, 80)}` });
  ...
  function snapshotNow() {
    ...
    a.click();
    log({ actorId: "admin", actorName: "Admin", entityType: "system", entityId: "snapshot", action: "admin.snapshot", summary: `Snapshot of ${rows.length} rows downloaded` });
  }
