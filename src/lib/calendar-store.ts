...
/* =====================================================================
   Visit War Room ↔ Calendar bridge — every visit gets a 1:1 cal event.
   Title format and description keep WhatsApp-paste-ready content.
   ===================================================================== */
...
export interface VisitEventInput {
  ...
  tcmEmail?: string;
  description: string;     // pre-rendered WhatsApp paste-ready block
  durationMin?: number;
}

/**
 * Idempotent: keyed by externalId = `visit:{tourId}`.
 * If an event for this tour exists, it's updated (reschedule / patch);
 * otherwise a new one is created. Returns the resulting event id.
 */
...
/** Patch only the description of an existing visit event (audit-log style). */
...
/**
 * Buffer guard — returns conflicting visit events for the given TCM in a
 * window around `scheduledAt`. Used by LeadActionDialog before booking.
 */
