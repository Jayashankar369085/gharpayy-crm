// Universal copy formatters — every "Copy" button in the CRM speaks the
// same WhatsApp-friendly shape. No tenant names in outbound messages.

import type { Lead, Property, TCM, Tour } from "@/lib/types";

// Reception / callback number the lead is told to dial 20 min before tour.
// Change in one place — every confirmation, reminder and reschedule picks it up.
...
/* ---------------- Outbound messages (no tenant name) ---------------- */
...
/* ---------------- Internal copy (for team handoff in WhatsApp) ---- */
...
/* =====================================================================
   ...
   Every chip in /visit-war renders one of these. Outbound text never
   includes the agent name — only the office phone, per project rule.
   ===================================================================== */

export interface VisitCopyCtx {
  leadName?: string;        // internal only — never appears in outbound text
  leadPhone?: string;
  propertyName: string;
  propertyArea: string;
...
/** Universal visit block — every confirmation / reschedule chip uses it. */
...
/** Owner-facing (anonymised — never carries lead name). */
...
/** Internal HR / Manager coaching ping — never customer-facing. */
