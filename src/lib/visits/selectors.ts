// Pure selectors over the visit war-room store. Keeping these out of the
// store file so multiple components (Day Planner, Team Pulse, War Map,
// Owner Portal, HR Coach Lens) can share one source of truth.
...
/* ──────────── Probability (deterministic, no AI) ──────────── */
...
/** 0..1 booking probability — used for revenue walking & expected bookings. */
...
export interface ZoneRollup {
  ...
  live: number;
  booked: number;
  walking: number;          // Σ price × probability
  topObjection?: string;
}
...
/* ──────────── Buffer collision detection (in-memory) ──────────── */
...
export function selectBufferConflicts(
  ...
  byTcm.forEach((list) => {
    for (let i = 1; i < list.length; i++) {
      const prev = list[i - 1];
      const curr = list[i];
      const prevEnd = prev.scheduledAt + 60 * 60_000; // assume 60-min slot
