...
/**
 * Visit Command Center store — the live war-room state for every visit.
 *
 * One record per tourId. Seeded lazily from the canonical tours array on
 * first read so existing scheduled tours show up immediately. All state is
 * local (Zustand + localStorage) — does NOT mutate the core lead/tour store.
 */
...
export interface ObjectionEntry {
  ...
  subType: string;          // "Deposit High", "Far From Office", etc.
  customerSaid: string;     // exact words
...
export interface VisitRecord {
  ...
  lastUpdateAt: number;
  escalated?: boolean;
  /** sticky badge already shown for this state — to avoid alert spam */
  warnedDelay?: boolean;
  ...
  /** owner code that owns the property (from ownerCodeForPG) */
  ownerCode?: string;
  /** colliding tourId if the buffer guard flagged this */
  bufferConflictWith?: string;
  /** HR / leadership flag — picked up by intervention queue */
...
export function probabilityFor(r?: Reaction, objections = 0, stage?: VisitStage): number {
  ...
  // Each unresolved objection trims confidence
...
export function timerColor(elapsedSec: number): string {
  if (elapsedSec >= 75 * 60) return "#FF2D2D"; // red
  if (elapsedSec >= 45 * 60) return "#FF5C00"; // orange
  if (elapsedSec >= 30 * 60) return "#FFD600"; // yellow
  return "#00FF85";                              // green
}
