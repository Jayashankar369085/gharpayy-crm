...
export function applyFilters(rows: AdminLeadRow[], f: AdminFilters): AdminLeadRow[] {
  ...
  out = [...out].sort((a, b) => {
    switch (field) {
      case "name": return mul * a.lead.name.localeCompare(b.lead.name);
      case "stage": return mul * a.lead.stage.localeCompare(b.lead.stage);
      case "prob": return mul * (a.probability - b.probability);
      case "value": return mul * (a.expectedValue - b.expectedValue);
      case "updated":
      default: return mul * (a.lastTouchTs - b.lastTouchTs);
    }
  });
  return out;
}
