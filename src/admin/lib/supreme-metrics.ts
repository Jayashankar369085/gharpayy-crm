/**
 * Supreme metrics — derived signals for the God-Mode Admin Supreme cockpit.
 * All pure functions over AdminLeadRow[].
 */
...
export interface MoneyMap {
  ...
  pipelineRevenue: number; // probability-weighted
  walkingRevenue: number;  // lost in last 30d
  atRiskRevenue: number;   // open + dormant + hot/warm with stale touch
  hotRevenue: number;      // probability >= 70
}
...
export interface TcmHealth {
  ...
  booked: number;
  lost: number;
  conversion: number;     // booked / (booked+lost)
  pipelineValue: number;
  avgAgeDays: number;
  loadScore: number;      // 0-100 utilization heuristic
