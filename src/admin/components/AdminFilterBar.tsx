...
export function AdminFilterBar({ filters, onChange, tcms, sources = [], stages = STAGES }: Props) {
  ...
  const savedViews: Record<string, AdminFilters> = useMemo(() => {
    ...
    try { return JSON.parse(localStorage.getItem("admin.views") ?? "{}"); } catch { return {}; }
  }, []);
    ...
    <div className="rounded-xl border border-border bg-card/60 p-3 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-1">
          <Input placeholder="Save view as…" value={savedViewName} onChange={(e) => setSavedViewName(e.target.value)} className="h-8 w-40 text-xs" />
          <Button size="sm" variant="outline" onClick={saveView} className="h-8 text-xs">Save</Button>
          {Object.keys(savedViews).length > 0 && (
            <Select onValueChange={(v) => onChange(savedViews[v])}>
              <SelectTrigger className="h-8 w-32 text-xs"><SelectValue placeholder="Load…" /></SelectTrigger>
              <SelectContent>
                {Object.keys(savedViews).map((k) => <SelectItem key={k} value={k}>{k}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      <ChipRow label="TCM" values={tcms.map((t) => t.id)} labels={Object.fromEntries(tcms.map((t) => [t.id, t.name]))}
        active={filters.assignedTo} onToggle={(v) => toggle("assignedTo", v)} />
      <ChipRow label="Zone" values={zones} active={filters.zone} onToggle={(v) => toggle("zone", v)} />
      {sources.length > 0 && (
        <ChipRow label="Source" values={sources} active={filters.source} onToggle={(v) => toggle("source", v)} />
      )}
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        <Button size="sm" variant={filters.hasVisit === true ? "default" : "outline"} className="h-7 text-xs"
          onClick={() => onChange({ ...filters, hasVisit: filters.hasVisit === true ? undefined : true })}>
          Has visit
        </Button>
        <Button size="sm" variant={filters.booked === true ? "default" : "outline"} className="h-7 text-xs"
          onClick={() => onChange({ ...filters, booked: filters.booked === true ? undefined : true })}>
          Booked
        </Button>
        <Button size="sm" variant={filters.booked === false ? "default" : "outline"} className="h-7 text-xs"
...
function ChipRow({
  label, values, active, onToggle, labels,
}: { label: string; values: readonly string[] | string[]; active: readonly string[]; onToggle: (v: string) => void; labels?: Record<string, string> }) {
